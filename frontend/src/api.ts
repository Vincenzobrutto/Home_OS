...
export const api = {
  ...
  houses: {
    get: (id: string) =>
      request<House & { rooms: Room[]; assets: Asset[] }>(`/houses/${id}`),
    create: (data: {
      ownerId: string;
      name: string;
      city?: string;
    ...
    update: (id: string, data: Partial<{ floorPlanRotation: number }>) =>
      request<House>(`/houses/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  },
  rooms: {
    ...
    create: (
      houseId: string,
      data: { type: string; name: string; planGeometry?: RoomGeometry },
    ) =>
      request<Room>(`/houses/${houseId}/rooms`, {
        method: 'POST',
    ...
    update: (
      id: string,
      data: Partial<{ type: string; name: string; planGeometry: RoomGeometry }>,
    ) =>
  ...
  assets: {
    listForHouse: (houseId: string) =>
      request<(Asset & { customFields: CustomField[] })[]>(
        `/houses/${houseId}/assets`,
      ),
    create: (
      houseId: string,
      data: {
    ...
    timeline: (id: string) =>
      request<TimelineEvent[]>(`/assets/${id}/timeline`),
    ...
    removeCustomField: (customFieldId: string) =>
      request<void>(`/custom-fields/${customFieldId}`, { method: 'DELETE' }),
    remove: (id: string) => request<void>(`/assets/${id}`, { method: 'DELETE' }),
    dismiss: (id: string) =>
      request<Asset>(`/assets/${id}/dismiss`, { method: 'POST' }),
    reactivate: (id: string) =>
      request<Asset>(`/assets/${id}/reactivate`, { method: 'POST' }),
    addTimelineEvent: (
      id: string,
      data: { eventDate: string; eventType: string; detail?: string; contactId?: string | null },
    ) =>
  ...
  contacts: {
    ...
    create: (
      houseId: string,
      data: { name: string; role?: string; phone?: string; email?: string; notes?: string },
    ) =>
      request<Contact>(`/houses/${houseId}/contacts`, {
        method: 'POST',
        body: JSON.stringify(data),
    ...
    update: (
      id: string,
      data: Partial<{ name: string; role: string; phone: string; email: string; notes: string }>,
    ) =>
  ...
  documents: {
    listForHouse: (houseId: string) =>
      request<DocumentRecord[]>(`/houses/${houseId}/documents`),
    ...
    analyze: (id: string) =>
      request<DocumentRecord>(`/documents/${id}/analyze`, { method: 'POST' }),
    confirm: (
      id: string,
      data: {
        assetId?: string;
    ...
    gmailCandidates: (houseId: string) =>
      request<GmailCandidate[]>(`/houses/${houseId}/gmail-candidates`),
    driveCandidates: (houseId: string) =>
      request<DriveCandidate[]>(`/houses/${houseId}/drive-candidates`),
    ...
    importCandidate: (id: string) =>
      request<DocumentRecord>(`/documents/${id}/import-candidate`, { method: 'POST' }),
    ignoreDocument: (id: string) =>
      request<DocumentRecord>(`/documents/${id}/ignore`, { method: 'POST' }),
    moveToHouse: (id: string) =>
      request<DocumentRecord>(`/documents/${id}/move-to-house`, { method: 'POST' }),
    searchOnline: (id: string) =>
      request<DocumentRecord>(`/documents/${id}/search-online`, { method: 'POST' }),
  },
  gmail: {
    ...
    disconnect: (userId: string) =>
      request<void>(`/users/${userId}/gmail-disconnect`, { method: 'POST' }),
  ...
  drive: {
    ...
    disconnect: (userId: string) =>
      request<void>(`/users/${userId}/drive-disconnect`, { method: 'POST' }),
    listFolders: (userId: string) =>
      request<DriveFolder[]>(`/users/${userId}/drive-folders`),
