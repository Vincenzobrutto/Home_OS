...
| B3 | `git init` del repository — oggi non è un repository Git | Alta | `.gitignore` root/backend/frontend già pronti (verificato 2026-08-02: escludono `node_modules`, `dist`, `coverage`, `.env*`, `uploads/`); prerequisito per changelog/decisions affidabili nel tempo e per qualunque collaborazione multi-assistente reale (branch, PR, blame) |
