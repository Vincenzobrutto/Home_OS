...
| B21 | Form piano di manutenzione (`Maintenance.tsx`): i campi "ogni N mesi/anni" e "giorni di preavviso" non hanno un'etichetta visibile sopra il campo — solo un `title` (tooltip) sul secondo, nessuna sul primo. Segnalato dall'utente su uno screenshot: senza contesto non è chiaro a cosa si riferiscano i due numeri | Media | aggiungere una label testuale sopra ciascun campo, coerente con lo stile già usato altrove nel form (es. "Ambiente" in `Assets.tsx`) |
